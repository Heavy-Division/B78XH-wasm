...
extern "C" {
	MSFS_CALLBACK bool MCPVSpeed_gauge_callback(FsContext ctx, int service_id, void* pData)
	{
		...
		case PANEL_SERVICE_PRE_INSTALL:
		{
			...
			g_MCPVSpeedVars.m_eDisplayedVerticalSpeedHoldValue = get_aircraft_var_enum("AUTOPILOT VERTICAL HOLD VAR");
		...
		case PANEL_SERVICE_PRE_DRAW:
		{
			...
			if (fVSpeedActive != 0) {
				FLOAT64 fVSpeed = aircraft_varget(g_MCPVSpeedVars.m_eDisplayedVerticalSpeedHoldValue, g_MCPVSpeedVars.m_eFeet, 0);
